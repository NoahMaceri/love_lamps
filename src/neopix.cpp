...
void NeoPix::rainbow()
{
  uint32_t time_out = millis() + NEOPIX_ANIM_LEN;
...
void NeoPix::crazy_rainbow() {
  // basically flash random colors
  uint32_t time_out = millis() + NEOPIX_ANIM_LEN;
  while (millis() < time_out) {
    for (uint8_t i = 0; i < NEOPIX_NUM_LEDS; i++) {
...
void NeoPix::pulse(CRGB color)
{
  ...
  while (millis() < time_out)
  {
    for (uint8_t i = 0; i < NEOPIX_BRIGHTNESS; i += 1)
    {
    ...
    for (uint8_t i = NEOPIX_BRIGHTNESS; i > 0; i -= 1)
    {
      fill_solid(leds, NEOPIX_NUM_LEDS, CRGB(color.r, color.g, color.b));
      FastLED.setBrightness(i);
      FastLED.show(); // Update the pixel display
...
void NeoPix::visor(CRGB color) {
  ...
  while (millis() < time_out) {
    ...
    for (uint8_t i = NEOPIX_NUM_LEDS - 1; i >= 0; i--) {
      leds[i] = CRGB(color.r, color.g, color.b);
      FastLED.show(); // Update the pixel display
      delay(NEOPIX_TRAIL_DELAY);
      leds[i] = CRGB::Black;
